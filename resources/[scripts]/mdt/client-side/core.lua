		...
		while true do
			...
			if IsControlJustPressed(1,38) and not Printing then
				...
				exports["screenshot-basic"]:requestScreenshotUpload(GetConvar("Discord_MDT"),"files[]",{ encoding = "webp", quality = 0.75 },function(Data)
					...
					local Url = Response and Response.attachments and Response.attachments[1] and Response.attachments[1].url or nil

					if not Url then
						TriggerEvent("Notify","Atenção","Falha ao enviar a foto do veículo. Verifique a configuração do <b>Discord_MDT</b> no server.cfg.","amarelo",5000)
						TriggerEvent("inventory:CloseButtons")
						SetFollowPedCamViewMode(Camera)
						return
					end

					SendNUIMessage({ Action = "SeizedVehicle", Payload = { VehicleName(Model),Plate,Passport,Name,FullRoad,Url } })
					TriggerEvent("inventory:CloseButtons")
					SetFollowPedCamViewMode(Camera)
					SetCursorLocation(0.5,0.5)
					TransitionToBlurred(1000)
