	...
	while true do
		...
		if not IsPedInAnyVehicle(Ped) then
			...
			for Number,v in pairs(Objects) do
				local IsPrisonTrash = v["Event"] == "farmer:Prison"
				local CanShow = #(Coords - v["Coords"]["xyz"]) <= (v["Show"] or 100.0) and GlobalState["Work"] >= GlobalState["Farmer:"..Number]

				-- Lixeiras de prisão só aparecem para quem está preso
				if IsPrisonTrash then
					CanShow = CanShow and LocalPlayer.state.Prison
				end
