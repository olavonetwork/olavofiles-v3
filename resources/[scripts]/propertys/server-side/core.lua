...
function Creative.Propertys(Name)
	...
	if CurrentTimer > Consult.Tax then
		...
		if vRP.Request(source,"Propriedades","Deseja pagar a hipoteca de <b>"..Currency..Dotted(Price).."</b>?") and vRP.PaymentFull(Passport,Price,true) then
...
AddEventHandler("propertys:Buy",function(Name)
	...
	local Payment = false
	if Mode == "Dollar" then
		Payment = vRP.PaymentFull(Passport,Informations[Interior].Price,true)
